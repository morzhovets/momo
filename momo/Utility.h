...
namespace internal
{
	...
	private:
		...
		static typename std::enable_if<size == 4, UInt>::type pvLog2(UInt value) MOMO_NOEXCEPT
		{
		...
		template<size_t size = sizeof(UInt)>
		static typename std::enable_if<size == 8, UInt>::type pvLog2(UInt value) MOMO_NOEXCEPT
		{
