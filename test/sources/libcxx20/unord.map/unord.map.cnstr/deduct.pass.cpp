...
int main(int, char**)
{
    ...
    {
    LIBCXX_TEST_CLASS<int, long, std::hash<long long>, std::equal_to<>, test_allocator<PC>> source;
    ...
    {
    LIBCXX_TEST_CLASS m({ P{1,1L}, P{2,2L}, P{1,1L}, P{INT_MAX,1L}, P{3,1L} }, 42, std::hash<long long>());
    ASSERT_SAME_TYPE(decltype(m), LIBCXX_TEST_CLASS<int, long, std::hash<long long>>);
    assert(std::is_permutation(m.begin(), m.end(), std::begin(expected_m), std::end(expected_m)));
    }

    {
    LIBCXX_TEST_CLASS m({ P{1,1L}, P{2,2L}, P{1,1L}, P{INT_MAX,1L}, P{3,1L} }, 42, std::hash<long long>(), std::equal_to<>());
    ASSERT_SAME_TYPE(decltype(m), LIBCXX_TEST_CLASS<int, long, std::hash<long long>, std::equal_to<>>);
    assert(std::is_permutation(m.begin(), m.end(), std::begin(expected_m), std::end(expected_m)));
    }

    {
    LIBCXX_TEST_CLASS m({ P{1,1L}, P{2,2L}, P{1,1L}, P{INT_MAX,1L}, P{3,1L} }, 42, std::hash<long long>(), std::equal_to<>(), test_allocator<PC>(0, 44));
    ASSERT_SAME_TYPE(decltype(m), LIBCXX_TEST_CLASS<int, long, std::hash<long long>, std::equal_to<>, test_allocator<PC>>);
    assert(std::is_permutation(m.begin(), m.end(), std::begin(expected_m), std::end(expected_m)));
    ...
    {
    LIBCXX_TEST_CLASS m({ P{1,1L}, P{2,2L}, P{1,1L}, P{INT_MAX,1L}, P{3,1L} }, 42, std::hash<long long>(), test_allocator<PC>(0, 48));
    ASSERT_SAME_TYPE(decltype(m), LIBCXX_TEST_CLASS<int, long, std::hash<long long>, std::equal_to<int>, test_allocator<PC>>);
    assert(std::is_permutation(m.begin(), m.end(), std::begin(expected_m), std::end(expected_m)));
    ...
    ASSERT_SAME_TYPE(decltype(m), LIBCXX_TEST_CLASS<int, int>);
    ...
    LIBCXX_TEST_CLASS m1{{std::pair{1, 2}, {3, 4}}, 0};
    ASSERT_SAME_TYPE(decltype(m1), LIBCXX_TEST_CLASS<int, int>);

    using value_type = std::pair<const int, int>;
    LIBCXX_TEST_CLASS m2{{value_type{1, 2}, {3, 4}}, 0};
    ASSERT_SAME_TYPE(decltype(m2), LIBCXX_TEST_CLASS<int, int>);
    }
...
#if TEST_STD_VER >= 23
    {
      ...
      { // (from_range, range)
        ...
        static_assert(std::is_same_v<decltype(c), LIBCXX_TEST_CLASS<int, long>>);
      }
      ...
      { // (from_range, range, n, hash)
        ...
        static_assert(std::is_same_v<decltype(c), LIBCXX_TEST_CLASS<int, long, Hash>>);
      }

      { // (from_range, range, n, hash, pred)
        LIBCXX_TEST_CLASS c(std::from_range, Range(), std::size_t(), Hash(), Pred());
        static_assert(std::is_same_v<decltype(c), LIBCXX_TEST_CLASS<int, long, Hash, Pred>>);
      }

      { // (from_range, range, n, hash, pred, alloc)
        LIBCXX_TEST_CLASS c(std::from_range, Range(), std::size_t(), Hash(), Pred(), Alloc());
        static_assert(std::is_same_v<decltype(c), LIBCXX_TEST_CLASS<int, long, Hash, Pred, Alloc>>);
      }

      { // (from_range, range, n, alloc)
        LIBCXX_TEST_CLASS c(std::from_range, Range(), std::size_t(), Alloc());
        static_assert(std::is_same_v<decltype(c), LIBCXX_TEST_CLASS<int, long, DefaultHash, DefaultPred, Alloc>>);
      }
      ...
      { // (from_range, range, n, hash, alloc)
        ...
        static_assert(std::is_same_v<decltype(c), LIBCXX_TEST_CLASS<int, long, Hash, DefaultPred, Alloc>>);
      }
    }
#endif

    //UnorderedContainerDeductionGuidesSfinaeAway<std::unordered_map, std::unordered_map<int, long>>();
    UnorderedContainerDeductionGuidesSfinaeAway<LIBCXX_TEST_CLASS, LIBCXX_TEST_CLASS<int, long>>();
