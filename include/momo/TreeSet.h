...
public:
	...
	size_t Insert(ArgIterator begin, ArgSentinel end)
	{
		...
		for (++iter; iter != end; ++iter)
		{
			...
			{
				pos = pvAdd<false>(std::next(pos),
					FastMovableFunctor(Creator<ItemArg>(memManager, std::forward<ItemArg>(ref))));
			}
		}
...
private:
	...
	Node* pvCopy(Node* srcNode)
	{
		...
		size_t itemIndex = 0;
		size_t childIndex = 0;
		for (internal::Finalizer fin(&TreeSetCore::pvDestroyExtra, *this, dstNode, itemIndex, childIndex);
			fin; fin.Detach())
		{
			MemManager& memManager = GetMemManager();
	...
	static Node* pvMergeFast(TreeSetCore& treeSet1, TreeSetCore& treeSet2)
	{
		...
		for (internal::Finalizer fin(&TreeSetCore::pvRevertMergeFast, treeSetPtr1); fin; fin.Detach())
		{
			while (true)
			{
