...
int main(int, char**)
{
    ...
    ASSERT_SAME_TYPE(decltype(m), momo::stdish::multimap<int, const long>);
    ...
    ASSERT_SAME_TYPE(decltype(m), momo::stdish::multimap<int, const long, std::greater<int>>);
    ...
    {
    const PCC arr[] = { {1,1L}, {2,2L}, {1,1L}, {INT_MAX,1L}, {3,1L} };
    momo::stdish::multimap m(std::begin(arr), std::end(arr), std::greater<int>(), test_allocator<PCC>(0, 42));

    ASSERT_SAME_TYPE(decltype(m), momo::stdish::multimap<int, const long, std::greater<int>, test_allocator<PCC>>);
    const PCC expected_m[] = { {INT_MAX,1L}, {3,1L}, {2,2L}, {1,1L}, {1,1L} };
    assert(std::equal(m.begin(), m.end(), std::begin(expected_m), std::end(expected_m)));
