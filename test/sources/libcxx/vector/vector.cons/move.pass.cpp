...
TEST_CONSTEXPR_CXX20 bool tests()
{
...
#endif
    {
      ...
      assert(is_contiguous_container_asan_correct(l));
      assert(is_contiguous_container_asan_correct(lo));
      vector<MoveOnly, safe_allocator<MoveOnly> > l2 = std::move(l);
      assert(l2 == lo);
      assert(l.empty());
      assert(l2.get_allocator() == lo.get_allocator());
      assert(is_contiguous_container_asan_correct(l2));
    ...
    {
      int a1[] = {1, 3, 7, 9, 10};
      vector<int, safe_allocator<int> > c1(a1, a1 + sizeof(a1) / sizeof(a1[0]));
      assert(is_contiguous_container_asan_correct(c1));
      vector<int, safe_allocator<int> >::const_iterator i = c1.begin();
      vector<int, safe_allocator<int> > c2                = std::move(c1);
      assert(is_contiguous_container_asan_correct(c2));
      vector<int, safe_allocator<int> >::iterator j = c2.erase(i);
      assert(*j == 3);
      assert(is_contiguous_container_asan_correct(c2));
    }
#endif
