...
public:
	...
	unordered_multimap_adaptor(size_type bucketCount, const hasher& hashFunc, const key_equal& equalComp,
		const allocator_type& alloc = allocator_type())
	...
	unordered_multimap_adaptor(std::initializer_list<value_type> values, size_type bucketCount,
		...
		const allocator_type& alloc = allocator_type())
		: unordered_multimap_adaptor(values.begin(), values.end(),
			bucketCount, hashFunc, equalComp, alloc)
	{
	}
	...
	unordered_multimap_adaptor& operator=(unordered_multimap_adaptor&& right)
		noexcept(momo::internal::ContainerAssignerStd::isNothrowMoveAssignable<unordered_multimap_adaptor>)
	{
	...
	iterator erase(const_iterator where)
	{
		...
		return momo::internal::ProxyConstructor<iterator>((keyIter->GetCount() > 1)
			? mHashMultiMap.Remove(iter) : mHashMultiMap.MakeIterator(mHashMultiMap.RemoveKey(keyIter)));
	}
	...
	iterator erase(const_iterator first, const_iterator last)
	{
		...
		if (first == last)
		{
			return momo::internal::ProxyConstructor<iterator>(
				mHashMultiMap.MakeMutableIterator(ConstIteratorProxy::GetBaseIterator(first)));
		}
		if (first != end())
		{
	...
	bool operator==(const unordered_multimap_adaptor& right) const
	{
...
private:
		...
		KeyIterator keyIter)
	{
		...
		if (count == 0)
			return { end, end };
		Iterator first = momo::internal::ProxyConstructor<Iterator>(hashMultiMap.MakeIterator(keyIter, 0));
		Iterator last = momo::internal::ProxyConstructor<Iterator>(hashMultiMap.MakeIterator(keyIter, count));
		return { first, last };
	}
