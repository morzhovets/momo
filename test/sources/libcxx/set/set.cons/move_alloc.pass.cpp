...
void main()
{
    ...
    }
    {
        ...
        Counter_base::gConstructed = 0;
        {
            ...
            assert(m3 == m2);
            LIBCPP_ASSERT(m1.empty());
            assert(Counter_base::gConstructed >= static_cast<int>(6+num));
            assert(Counter_base::gConstructed <= static_cast<int>(m1.size()+6+num));

            {
            M m4(std::move(m2), A(5));
            assert(Counter_base::gConstructed >= static_cast<int>(6+num));
            assert(Counter_base::gConstructed <= static_cast<int>(m1.size()+m2.size()+6+num));
            assert(m4 == m3);
            LIBCPP_ASSERT(m2.empty());
            }
            assert(Counter_base::gConstructed >= static_cast<int>(3+num));
            assert(Counter_base::gConstructed <= static_cast<int>(m1.size()+m2.size()+3+num));
        }
